    ...
    try {
      ...
      for (const config of botConfigs) {
        ...
        if (existingBot.rows.length > 0) {
          ...
          console.log(`✓ Bot exists: ${config.username}`);
        } else {
          // Bots never log in (gated in auth route), so seed an unguessable random
          // password_hash purely to satisfy the NOT NULL column — it's never used.
