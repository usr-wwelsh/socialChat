...
### 3. Add PostgreSQL Database (Optional) / Volume (for SQLite) / Volume (for Sessions)
...
-   **For Persistent Sessions:** The application uses file-based sessions, which are stored in the `.sessions` directory at the project root. For these sessions to persist across deployments/restarts on Railway, you must add another "Volume" service with the mount path `/app/.sessions`.
...
├── .sessions/                # File-based session storage (generated by express-session)
└── README.md
```
...
-   `POST /api/moderation/posts/:postId/pin` - Pin a post (unpins any previously pinned post)
-   `DELETE /api/moderation/posts/:postId/pin` - Unpin a post
